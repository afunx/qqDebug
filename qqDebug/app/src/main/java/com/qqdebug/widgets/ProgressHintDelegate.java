...
public abstract class ProgressHintDelegate implements OnSeekBarChangeListener {
  ...
  private int mPopupStep = 1;
  private int mPopupMin = 0;
  ...
  private onSeekBarProgress mSeekBarProgress;
  ...
  public ProgressHintDelegate(SeekBar seekBar, int mPopupLayout, int mPopupOffset,
    ...
    initDelegate(seekBar, mPopupLayout, mPopupOffset, mPopupAlwaysShown, mPopupDraggable,
        mPopupStyle, mPopupAnimStyle, DEFAULT_HINT_ADAPTER);
  }
  ...
  public ProgressHintDelegate(SeekBar seekBar, AttributeSet attrs, int defStyleAttr) {
    ...
    mPopupStep = a.getInt(R.styleable.ProgressHint_popupStep, 1);
    mPopupMin = a.getInt(R.styleable.ProgressHint_popupMin, 0);
    Log.e("Test","属性中的最小值="+mPopupMin);
    a.recycle();
    //
    initDelegate(seekBar, mPopupLayout, mPopupOffset, mPopupAlwaysShown, mPopupDraggable,
        mPopupStyle, mPopupAnimStyle, DEFAULT_HINT_ADAPTER);
  ...
  @Override public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
    String popupText = null;
    int realProgress = (progress+mPopupMin);
    Log.e("Test","progress="+progress+"----mPopupMin"+mPopupMin);
    int display = mPopupStep==1?realProgress:(realProgress-realProgress%mPopupStep);
    if (mHintAdapter != null) {
      popupText = mHintAdapter.getHint(mSeekBar, display);
    }
    mPopupTextView.setText(popupText != null ? popupText : String.valueOf(display));
    if(mSeekBarProgress!=null)
      mSeekBarProgress.onProgress(seekBar, display,fromUser);
  ...
  public void setOnProgressListener(onSeekBarProgress onProgressListener){
    this.mSeekBarProgress = onProgressListener;
  }
  ...
  public void setPopupMin(int min){
    Log.e("Test","设置最小值="+min);
    this.mPopupMin=min;
  }
}
